...
export class IncomingTextFormComponent implements OnInit {
    ...
    onUploadFile(event) {
        this.incomingTextModel.file = event.target?.files[0]
    }

    onSave() {
        if (this.isEdit)
            this.incomingTextService.updateIncomingText(this.incomingTextModel, this.incomingTextModel.id).subscribe(res => {
            ...
            this.incomingTextService.createIncomingText(this.incomingTextModel).subscribe(res => {
                this.messageService.add({
                    severity: 'success',
