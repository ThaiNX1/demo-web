...
export class TimekeepingComponent implements OnInit {
    ...
    updateTimeKeeper(data: TimeKeeping) {
        ...
        data.symbolId = data.symbolCode
            ...
            : null;
        data.targetCode = this.listTarget.find((item) => item.id === data.targetId)?.code;
        console.log(data.targetCode)
        data.userFullName = data.fullName;
        data.userName = data.username;
        data.timeIn = moment(this.getParams.dateTimeKeep).format(this.appConstant.FORMAT_DATE.MOMENT_T_DATE);
        data.timeIn = moment(this.getParams.dateTimeKeep).format(this.appConstant.FORMAT_DATE.MOMENT_T_DATE);
        data.checked = true;
        data.isOverTime = 0;
    ...
    updateTimeKeeperWith(data: TimeKeeping) {
        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                ...
                data.userName = data.username;
                data.timeIn = moment(this.getParams.dateTimeKeep).format(this.appConstant.FORMAT_DATE.MOMENT_T_DATE);
                data.timeOut = moment(this.getParams.dateTimeKeep).format(this.appConstant.FORMAT_DATE.MOMENT_T_DATE);
                data.checked = true;
                data.isOverTime = 0;
                this.TimeKeepingService.saveInOut(data).subscribe(() => {
                    this.getTimekeeping();
